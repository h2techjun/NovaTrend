...
export default function PostDetailPage() {
  ...
  const fetchPost = useCallback(async () => {
    const { data, error } = await supabase
      .from('posts')
      .select('*, author:profiles!posts_author_id_fkey(id, username, display_name, avatar_url, plan)')
      .eq('id', Number(id))
      .eq('is_deleted', false)
      .single();
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-[hsl(var(--border))]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">{post.author?.display_name || post.author?.username || '익명'}</p>
